...
image(matrix(train_x[145,],28,28,byrow=T), axes = FALSE,col=gray.colors(255))
...
# # visualize the built model (implies installing some python packages first) 
# library(kerasR)
# library(reticulate)
...
# 
# # install pydot
# conda_install("r-reticulate", "pydot")
# conda_install("r-reticulate", "graphviz")
# 
# # import pydot (it will be automatically discovered in "r-reticulate")
# pydot <- import("pydot")
...
res <- apply(prediction, 1, FUN="which.max")-1

image(matrix(test_x[5,],28,28,byrow=T), axes = FALSE,col=gray.colors(255))
# vizualization of the results (need to improve)
image(as.matrix(prediction[1:5,]))
head(res)
true <- apply(test_y, 1, FUN="which.max")-1
head(true)

# get the classes directly
res2 <- predict_classes(model, test_x)
head(res2)